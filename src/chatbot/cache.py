...
def get_cached(question: str, location_uuid: str | None) -> str | None:
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT answer FROM cache_chatbot WHERE cache_key = %s AND expires_at > NOW()",
        [k],
    ).fetchone()
    if not row:
        return None
    conn.execute(
        "UPDATE cache_chatbot SET hits = hits + 1 WHERE cache_key = %s",
        [k],
    )
    return row[0]
...
def set_cached(question: str, location_uuid: str | None, answer: str) -> None:
    ...
    conn.execute(
        """
        INSERT INTO cache_chatbot (cache_key, question, location_uuid, answer, expires_at)
        VALUES (%s, %s, %s, %s, %s)
...
def purge_expired() -> int:
    ...
    conn = get_conn()
    n = conn.execute("SELECT COUNT(*) FROM cache_chatbot WHERE expires_at <= NOW()").fetchone()[0]
    conn.execute("DELETE FROM cache_chatbot WHERE expires_at <= NOW()")
    return n

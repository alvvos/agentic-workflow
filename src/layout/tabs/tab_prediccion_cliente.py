...
def _zona_card(nombre: str, res: dict, color: str) -> dbc.Col:
    ...
    if reales_val and predichos:
        ...
        if media_real > 0:
            ...
            if abs(pct) < 3:
                tendencia_el = html.Span('= Sin cambios significativos',
    ...
    return dbc.Col(
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div([
                    html.Div(f"{dia_lbl} {fecha_lbl}", className='text-muted mb-0',
                             style={'fontSize': '0.72rem'}),
                    html.Div(f"{primera_val:,}",
                             className='fw-bold text-dark lh-1',
                             style={'fontSize': '1.9rem'}),
                    html.Div('visitas previstas', className='text-muted mb-1',
                             style={'fontSize': '0.70rem'}),
                    tendencia_el,
                ], className='mb-3'),

                # Mini gráfico
                dcc.Graph(figure=fig, config=_CFG, style={'height': '160px', 'marginX': '-4px'}),
            ], className='p-3'),
...
def _empty_state(msg: str = '') -> html.Div:
    return html.Div([
        ...
        html.P(msg or 'Selecciona una ubicación en el panel izquierdo para ver la previsión.',
...
def _loc_section(loc_nombre: str, zona_cols: list) -> html.Div:
    return html.Div([
        html.Div([
            html.I(className='fas fa-location-dot me-2 text-primary', style={'fontSize': '0.9rem'}),
    ...
    return dcc.Tab(
        ...
        children=[html.Div([
            dbc.Row([
                dbc.Col([
                    html.H4([
                        html.I(className='fas fa-wand-magic-sparkles me-2 text-primary'),
...
def actualizar_prediccion_publica(tab, locs, session_id):
    ...
    return html.Div([
        ...
        html.Div([
            html.I(className='fas fa-circle-info me-1 text-muted', style={'fontSize': '0.75rem'}),
            html.Span(
        ...
        ], className='mb-4 p-3 rounded-3', style={'background': '#f8f9fa'}),
        html.Div(secciones),
    ])

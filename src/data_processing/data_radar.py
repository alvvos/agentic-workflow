...
def generar_tabla_auditoria(df_filt):
    ...
    leyenda = html.Div(
        [
            ...
            html.Span(
                [html.I(className="fas fa-check-circle me-1"), "Normal"],
                className="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill px-3 py-2 me-2",
            ),
            html.Span(
                [html.I(className="fas fa-exclamation-triangle me-1"), "Aviso"],
                className="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 rounded-pill px-3 py-2 me-2",
            ),
            html.Span(
                [html.I(className="fas fa-times-circle me-1"), "Anomalía"],
                className="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 rounded-pill px-3 py-2 me-2",
            ),
            html.Span(
                [html.I(className="fas fa-calendar-alt me-1"), "Festivo"],
    ...
    for ubi in df_filt["Ubicación"].unique():
        ...
        for year, month in sorted(df_ubi["year_month"].unique()):
            ...
            dias_totales = (
                (ultimo_dia + timedelta(days=6 - ultimo_dia.weekday())) - lunes_inicial
            ).days + 1
            ...
            cabeceras = [
                html.Div(
                    dia,
                    className="text-center fw-bold py-2 text-secondary bg-light rounded-top-3 small text-uppercase",
                )
                for dia in [
                    "Lunes",
                    "Martes",
            ...
            for fecha_actual in rango_fechas:
                if fecha_actual.month != month:
                    celdas_calendario.append(
                        html.Div(
                            className="bg-light opacity-50 rounded-4 border-0",
                ...
                if fecha_str in clima_datos:
                    tmax, tmin, precip = (
                        clima_datos[fecha_str].get("tmax"),
                        clima_datos[fecha_str].get("tmin"),
                        clima_datos[fecha_str].get("precip", 0),
                    )

                icon_class, icon_color, desc_clima = "fas fa-cloud-sun", "#95a5a6", "Poco nuboso"
                if precip and precip > 1.0:
                    icon_class, icon_color, desc_clima = (
                        "fas fa-cloud-showers-heavy",
                        "#3498db",
                        f"Lluvia ({precip}mm)",
                    )
                elif tmax and tmax >= 25:
                    icon_class, icon_color, desc_clima = "fas fa-sun", "#f39c12", "Calor"
                elif tmax and tmax < 12:
                    icon_class, icon_color, desc_clima = "fas fa-snowflake", "#3498db", "Frío"

                clima_ui = (
                    html.Span(
                        f"{round(tmax)}°/{round(tmin)}°", className="small text-muted fw-bold"
                    )
                ...
                header_left = html.Div(
                    [
                        ...
                        html.I(
                            ...
                            title=desc_clima,
                            style={"color": icon_color, "fontSize": "14px"},
                        ),
                        clima_ui,
                    ],
                ...
                if es_festivo:
                    header_elements.append(
                        html.Span(
                            festivos_locales.get(fecha_actual),
                            className="small fw-bold text-primary text-end ms-2",
                            style={"lineHeight": "1.1", "maxWidth": "50%"},
                        )
                    )
                    ...
                    celdas_calendario.append(
                        html.Div(
                            [
                                html.Div(
                                    header_elements,
                                    className=f"p-2 d-flex justify-content-between align-items-center rounded-top-4 {'bg-primary bg-opacity-10' if es_festivo else 'bg-light'}",
                                ),
                                html.Div(
                                    "Sin datos.", className="p-3 small text-muted text-center"
                                ),
                            ],
                            className="bg-white border shadow-sm rounded-4",
                            style={"minHeight": "140px"},
                        )
                    )
                    continue
                ...
                for _, row in df_dia.iterrows():
                    ...
                    tv, uv, nv, dt = (
                        ...
                        row.get("new_visitors", 0),
                        row.get("dwell_time", 0),
                    )
                    if dt == 0 and (tv == 0 or uv == 0):
                        inactivos.append(zona)
                    elif uv > tv:
                        errores_mat.append(f"Únicos > Totales ({zona})")
                    elif nv > uv:
                        errores_mat.append(f"Nuevos > Únicos ({zona})")
                    elif dt > 0 and tv == 0:
                        errores_mat.append(f"Estancia fantasma ({zona})")

                df_agrupado = df_dia.groupby("Zona")["unique_visitors"].sum().reset_index()
                zonas_dict = dict(
                    zip(df_agrupado["Zona"].str.lower(), df_agrupado["unique_visitors"])
                )
                ...
                if caja_val > 0 and calle_val > 0 and caja_val > calle_val:
                    errores_fis.append("Caja > Exterior")
                if caja_val > 0 and tienda_val > 0 and caja_val > tienda_val:
                    errores_fis.append("Caja > Interior")
                ...
                if inactivos:
                    ...
                    mensajes.append(
                        (
                            f"Inactivo: {', '.join(set(inactivos))}",
                            "warning",
                            "fas fa-exclamation-triangle",
                        )
                    )
                else:
                    mensajes.append(("Sensores OK", "success", "fas fa-check-circle"))

                if errores_mat or errores_fis:
                    estado_dia = "danger"
                    for err in set(errores_mat + errores_fis):
                        mensajes.append((err, "danger", "fas fa-times-circle"))
                elif not inactivos:
                    mensajes.append(("Flujo coherente", "success", "fas fa-check-circle"))

                bg_color, bg_header, border_class = "bg-white", "bg-light", "border-light"
                if estado_dia == "danger":
                    bg_color, bg_header, border_class = (
                ...
                if es_festivo:
                    bg_header = "bg-primary bg-opacity-10"

                celda_content = [
                    html.Div(
                        header_elements,
                        className=f"p-2 d-flex justify-content-between align-items-center rounded-top-4 {bg_header}",
                    )
                ]
                lista_html = [
                    html.Div(
                        [html.I(className=f"{icon} me-2"), html.Span(msg)],
                        className=f"small fw-bold text-{tipo} mb-1",
                    )
                ...
                celdas_calendario.append(
                    html.Div(
                        celda_content,
                        className=f"{bg_color} border {border_class} shadow-sm rounded-4",
                        style={"minHeight": "140px"},
                    )
                )

            grilla = html.Div(
                cabeceras + celdas_calendario,
                style={"display": "grid", "gridTemplateColumns": "repeat(7, 1fr)", "gap": "15px"},
            )
            tabs_meses.append(
                dbc.Tab(
                    html.Div(grilla, className="pt-4"),
                    label=nombre_mes,
                    tab_id=nombre_mes,
                    className="fw-bold",
                )
            ...
            dbc.Card(
                dbc.CardBody(
                    [
                        ...
                        (
                            dbc.Tabs(tabs_meses, active_tab=tabs_meses[0].tab_id)
                            if tabs_meses
                            else html.Div()
                        ),
                    ]
    ...
    return html.Div(
        [
            html.H4(
                [
                    html.I(className="fas fa-calendar-check me-2 text-primary"),
                    "Calendario de Auditoría Operativa",
                ],
                className="fw-bold mb-3 mt-5 text-dark",
            ),
            leyenda,
        ]
        + paneles_ubicacion

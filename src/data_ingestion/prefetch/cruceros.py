...
def _fetch_month(month: int, year: int) -> list[dict]:
    ...
    for row in rows[1:]:
        ...
        partes     = [p.strip() for p in buque_raw.split() if p.strip()]
        buque      = buque_raw
...
def _parse_arrival_date(entrada_salida: str, query_month: int, query_year: int) -> Optional[date]:
    """
    col[4] = "DD/MM HH:MM<br>DD/MM HH:MM" — toma la fecha de llegada (primera parte).
    El año se infiere del mes consultado.
    """
    try:
        ...
        # Si el mes de llegada es anterior al mes consultado, sigue siendo el mismo año
        # (ej: barco que llega el 28/05 aparece en la consulta de 06/2026)
        year = query_year
        if month > query_month + 1:
            year -= 1
...
def run(
    ...
    try:
        ...
        if verbose:
            ...
            print(f'  [cruceros] Málaga Muelle 1: {n} escalas  '
...
if __name__ == '__main__':
    ...
    parser.add_argument('--desde',   default=None, metavar='YYYY-MM',
                        help='Mes de inicio (ej: 2025-01). Por defecto: mes actual')
    parser.add_argument('--hasta',   default=None, metavar='YYYY-MM',

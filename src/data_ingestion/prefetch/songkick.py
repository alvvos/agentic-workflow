"""
Prefetch eventos Songkick — conciertos y festivales.

Requiere SONGKICK_KEY en .env. Sin key → omite silenciosamente.

Escribe en store_features_ext:
  ev_rank_concierto, ev_rank_festival  (0-100)
"""
...
from src.data_processing.fuentes_eventos.songkick import (
...
def _run_one(loc: dict, verbose: bool) -> int:
    ...
    n = len(daily)
    if verbose:
        print(f"  [songkick] {loc['nombre']}: {n}d con eventos  ({len(raw)} raw)")
    return n
...
def run(
    ...
    if not _key():
        if verbose:
            print("  [songkick] SONGKICK_KEY no configurada — omitido")
        return {}
    ...
    for loc in locations:
        ...
        if is_fresh(uuid, _SOURCE, max_age_hours):
            if verbose:
                print(f"  [songkick] {loc['nombre']}: omitido (datos < {max_age_hours:.0f}h)")
            stats[uuid] = 0
            continue
        try:
            ...
            if verbose:
                print(f"  [songkick] {loc['nombre']}: ERROR — {e}")
            stats[uuid] = 0
...
if __name__ == '__main__':
    ...
    args = make_parser('eventos Songkick').parse_args()
    run(

...
from src.data_ingestion.prefetch._common import get_active_locations
    ...
    if is_primera:
        ...
        if tiene_placeholder:
            ...
            conn.executemany(
                ...
                [
                    (location_uuid, col, _TRAINING_START,
...
def listar_estado(verbose: bool = True) -> list[dict]:
    ...
    if verbose:
        ...
        print(f"{'─'*56}")
        for r in result:
            bar = '█' * r['features_con_dato'] + '░' * (r['features_total'] - r['features_con_dato'])
            print(f"  {r['location_uuid'][:8]}  {bar}  "

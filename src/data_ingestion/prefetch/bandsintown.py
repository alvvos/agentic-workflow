"""
Prefetch eventos Bandsintown — conciertos por ubicación.

Requiere BANDSINTOWN_KEY en .env. Sin key → omite silenciosamente.

Escribe en store_features_ext:
  ev_rank_concierto  (0-100)
"""
...
from src.data_processing.fuentes_eventos.bandsintown import (
    fetch_events_raw, events_to_daily_scores, events_to_raw_rows, _key,
)
...
def _run_one(loc: dict, verbose: bool) -> int:
    ...
    raw    = fetch_events_raw(lat, lon, date_from, date_to, city=loc.get('city', ''))
    ...
    for d, cats in scores.items():
        if date_from <= d <= date_to:
            daily[d] = {'ev_rank_concierto': cats.get('concierto', 0)}
    ...
    n = len(daily)
    if verbose:
        print(f"  [bandsintown] {loc['nombre']}: {n}d con eventos  ({len(raw)} raw)")
    return n
...
def run(
    ...
    if not _key():
        if verbose:
            print("  [bandsintown] BANDSINTOWN_KEY no configurada — omitido")
        return {}
    ...
    for loc in locations:
        ...
        if is_fresh(uuid, _SOURCE, max_age_hours):
            if verbose:
                print(f"  [bandsintown] {loc['nombre']}: omitido (datos < {max_age_hours:.0f}h)")
            stats[uuid] = 0
            continue
        try:
            ...
            if verbose:
                print(f"  [bandsintown] {loc['nombre']}: ERROR — {e}")
            stats[uuid] = 0
...
if __name__ == '__main__':
    ...
    args = make_parser('eventos Bandsintown').parse_args()
    run(

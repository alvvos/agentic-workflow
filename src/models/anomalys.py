...
def construir_figura_bi(
    ...
    if df.empty:
        return fig.update_layout(
            title="Sin datos", xaxis=dict(visible=False), yaxis=dict(visible=False)
        )
    ...
    if usar_meses:
        ...
        ticktext = [
            f"{_MESES_ES[pd.to_datetime(f).month - 1]} {pd.to_datetime(f).year}"
    ...
    for zona in zonas_ordenadas:
        ...
        if has_historical:
            ...
            if not df_z_hist.empty:
                df_z_hist["fecha_alineada"] = df_z_hist["fecha_dia"] + pd.Timedelta(
                    days=offset_dias
                )
        ...
        if tipo == "bar":
            if not df_z_hist.empty:
                ...
                fig.add_trace(
                    go.Bar(
                        x=df_z_hist["fecha_alineada"],
                        y=df_z_hist[metrica],
                        name=f"{zona} (Ant.)",
                        marker=dict(
                            color=color_zona,
                            opacity=0.15,
                            line=dict(color=color_zona, width=1),
                            cornerradius=5,
                        ),
                        hoverinfo="skip",
                        showlegend=False,
                        customdata=[zona] * len(df_z_hist),
                    )
                )
            bar_text = [
                f"{int(round(v)):,}" if (mostrar_numeros and pd.notna(v) and v > 0) else ""
                for v in df_z[metrica]
            ]
            fig.add_trace(
                go.Bar(
                    x=df_z["fecha_dia"],
                    y=df_z[metrica],
                    name=zona,
                    marker=dict(color=color_zona, opacity=0.9, cornerradius=5),
                    hoverinfo="text",
                    ...
                    constraintext="none",
                    textfont=dict(size=10, color="white", family="Arial Black, Arial, sans-serif"),
                )
            )
        else:
            if not df_z_hist.empty:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        name=f"{zona} (Ant.)",
                        mode="lines",
                        line=dict(color=color_zona, width=1.5, dash="dot"),
                        hoverinfo="skip",
                        showlegend=False,
                        opacity=0.4,
                        customdata=[zona] * len(df_z_hist),
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=zona,
                    mode="lines+markers",
                    line=dict(color=color_zona, width=3, shape="spline"),
                    marker=dict(size=7),
                    hoverinfo="text",
                    hovertext=hover_texts,
                    customdata=[zona] * len(df_z),
            ...
            if len(df_z) > 2:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=trend,
                        mode="lines",
                        line=dict(color=color_zona, width=1.5, dash="dash"),
                        opacity=0.45,
                        showlegend=False,
                        hoverinfo="skip",
                        customdata=[zona] * len(df_z),
                ...
                if not p_pos.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                color="#27ae60", size=12, symbol="circle-open", line=dict(width=3)
                            ),
                            showlegend=False,
                            hoverinfo="skip",
                            customdata=[zona] * len(p_pos),
                        )
                    )
                ...
                if not p_neg.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                color="#e74c3c", size=12, symbol="x-open", line=dict(width=3)
                            ),
                            showlegend=False,
                            hoverinfo="skip",
                            customdata=[zona] * len(p_neg),
                        )
                    )

    fig.update_layout(
        title=dict(text=titulo, font=dict(size=15, color="#2c3e50", family="Arial, sans-serif")),
        plot_bgcolor="white",
        hovermode="x unified",
        margin=dict(t=45, b=45, l=40, r=20),
        legend=dict(orientation="h", y=1.1, x=1, xanchor="right"),
        barmode="group" if tipo == "bar" else None,
        clickmode="event+select",
        dragmode=False,
    )
    fig.update_xaxes(
        tickvals=tickvals, ticktext=ticktext, showgrid=True, gridcolor="#f2f2f2", tickangle=angulo_x
    )
...
def crear_tarjeta_metrica(
    df, df_hist, col_y, titulo, offset, ubi, id_sufijo, tipo="bar", festivos=None
):
    zonas = ordenar_zonas(df["Zona"].unique())
    fig = construir_figura_bi(
        df, df_hist, col_y, titulo, zonas, obtener_mapa_colores(zonas), tipo, offset, festivos
    )
    gid = f"{ubi}-{col_y}-{id_sufijo}"
    cfg = {
        **_GRAPH_CONFIG,
        "toImageButtonOptions": {**_GRAPH_CONFIG["toImageButtonOptions"], "filename": gid},
    }
    return dbc.Card(
        [
            dbc.CardHeader(
                [
                    dbc.Button(
                        ...
                        id={"type": "btn-expand", "index": gid},
                        color="link",
                        className="p-0 text-muted float-end ms-2",
...
def _seccion_uv_rolling(df_z, df_zh, cols_uv, multi_mes, zona, color_zona, ubi=""):
    ...
    if not multi_mes:
                    ...
                    dbc.Card(
                        dbc.CardBody(
                            [
                                ...
                                html.H4(
                                    f"{val:,.0f}", className="fw-bold mb-0", style={"color": color}
                                ),
                            ],
        ...
        return html.Div(
            [
                html.Small(
                    "Visitantes únicos rolling",
                    className="text-muted text-uppercase fw-bold d-block mt-3 mb-2",
                ),
                dbc.Row(uv_cards),
            ]
        )
        ...
        for col in cols_uv:
            ...
            for mes in meses:
                ...
                serie = (
                    df_mes.sort_values("fecha_dia")[col].dropna()
            ...
            if vals:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        marker=dict(
                            color=colores_uv.get(col, "#7f8c8d"), opacity=0.85, cornerradius=5
                        ),
                        text=bar_labels,
                        textposition="inside",
                        insidetextanchor="middle",
                        constraintext="none",
                        textfont=dict(
                            size=11, color="white", family="Arial Black, Arial, sans-serif"
                        ),
                    )
                )
        fig.update_layout(
        ...
        return dbc.Card(
            [
                dbc.CardHeader(
                    [
                        dbc.Button(
                            html.I(className="far fa-square"),
                            id={"type": "btn-select-graph", "index": gid},
                            color="link",
...
def crear_tarjeta_kpi_global(titulo, val_actual, val_hist, es_tiempo=False, es_ratio=False):
    ...
    if val_hist is not None and val_hist > 0:
        ...
        color = (
            "text-success" if delta > 0.01 else ("text-danger" if delta < -0.01 else "text-muted")
        )
        flecha = "▲" if delta > 0.01 else ("▼" if delta < -0.01 else "◼")

        valor_evol = f"{delta:+.2f} pp" if es_ratio else f"{pct:+.1f}%"
        delta_html = html.Span(
            f"{flecha} {valor_evol} vs Ant. ({txt_hist})", className=f"small fw-bold {color}"
        )
...
def generar_panel_bi_completo(
    ...
    if fechas_filtro:
        ...
        if df_actual.empty:
            return dbc.Alert(
                "Ningún dato coincide con los días seleccionados.",
                color="info",
                className="rounded-4",
            )
    ...
    for ubi in df_actual["Ubicación"].unique():
        ...
        for zona in zonas_top:
            ...
            hist_tv = (
                df_zh["total_visits"].sum()
                if not df_zh.empty and "total_visits" in df_zh.columns
                else None
            )
            hist_uv = (
                df_zh["unique_visitors"].sum()
                if not df_zh.empty and "unique_visitors" in df_zh.columns
                else None
            )
            hist_nv = (
                df_zh["new_visitors"].sum()
                if not df_zh.empty and "new_visitors" in df_zh.columns
                else None
            )
            hist_dt = (
                df_zh["dwell_time"].mean()
                if not df_zh.empty and "dwell_time" in df_zh.columns
                else None
            )
        ...
        if len(zonas_top) > 1:
            ...
            for i in range(len(zonas_top) - 1):
                ...
                if not df_r_act.empty:
                    ...
                    kpis_funnel.append(
                        dbc.Col(
                            crear_tarjeta_kpi_global(titulo_f, v_ratio, h_ratio, es_ratio=True),
                            xs=12,
                            md=6,
                            className="mb-3",
                        )
                    ...
                    card_g = crear_tarjeta_metrica(
                        df_r_act,
                        df_r_hist,
                        "ratio_atraccion",
                        f"Evolución {titulo_f}",
                        offset,
                        ubi,
                        f"funnel-{i}",
                        "line",
                        festivos=festivos_ubi,
                    )
        ...
        df_int = (
            df_u[~m_ext]
            .groupby(["fecha_dia", "Zona"])
            .agg({k: v for k, v in agg.items() if k in df_u.columns})
            .reset_index()
            if not df_u[~m_ext].empty
            else pd.DataFrame()
        )
        df_ext = (
            df_u[m_ext]
        ...
        if not df_h.empty:
            ...
            df_hi = (
                df_h[~m_hex]
                .groupby(["fecha_dia", "Zona"])
                .agg({k: v for k, v in agg.items() if k in df_h.columns})
                .reset_index()
                if not df_h[~m_hex].empty
                else pd.DataFrame()
            )
            df_he = (
                df_h[m_hex]
        ...
        for i in range(0, len(cards), 2):
            ...
            col2 = (
                dbc.Col(cards[i + 1], xs=12, xl=6, className="mb-4")
                if i + 1 < len(cards)

...
def _serie_val(p, d: date, rng) -> float:
    ...
    return max(
        0.0,
        round(base * (1 + p["amp"] * sin(2 * pi * (doy - 80) / 365) + rng.gauss(0, p["noise"]))),
    )
...
def main():
    ...
    cal_rows_db = [(org_uuid, r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8]) for r in cal_rows]
    conn.executemany(
    ...
    print(
        f"OK — {len(serie_rows)} filas de serie + {len(cal_rows)} eventos escritos para Showroom."
    )
